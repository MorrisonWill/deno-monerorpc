...
export class MoneroDaemonRPC {
  ...
  async get_block_template(wallet_address: string, reserve_size: number) {
    return this.request("get_block_template", [wallet_address, reserve_size], [
      "wallet_address",
      "reserve_size",
    ]);
  }
  ...
  async get_block_headers_range(start_height: number, end_height: number) {
    return this.request("get_block_headers_range", [start_height, end_height], [
      "start_height",
      "end_height",
    ]);
  }
  ...
  async set_bans(
    bans: Array<{ host: string; ip: number; ban: boolean; seconds: number }>,
  ) {
  ...
  async get_output_histogram(
    ...
    return this.request("get_output_histogram", [
      amounts,
      min_count,
      max_count,
      unlocked,
  ...
  async get_coinbase_tx_sum(height: number, count: number) {
    return this.request("get_coinbase_tx_sum", [height, count], [
      "height",
      "count",
    ]);
  }
  ...
  async get_output_distribution(
    amounts: Array<number>,
    cumulative?: number,
    from_height?: number,
    to_height?: number,
  ) {
    return this.request("get_output_distribution", [
      amounts,
      cumulative,
      from_height,
      to_height,
    ], ["amounts", "cumulative", "from_height", "to_height"]);
  }
}
